...
# Runs on first container boot via supervisord (reference-install program).
# Idempotent: exits immediately (0) if the reference is already installed.
set -euo pipefail
...
LOCK_FILE="$FLEET_DIR/reference/.installing"
...
if [ -f "$REFERENCE_DIR/terminal64.exe" ] && [ -f "$WINEPREFIX/drive_c/Python311/python.exe" ]; then
    echo "[install] Reference already installed. Skipping."
    exit 0
fi

# -- Acquire lock file --------------------------------------------------------
mkdir -p "$FLEET_DIR/reference"
touch "$LOCK_FILE"
trap 'rm -f "$LOCK_FILE"' EXIT

# -- Wait for reference display (:99) to be ready ----------------------------
echo "[install] Waiting for display $DISPLAY ..."
for i in $(seq 1 60); do
    xdpyinfo -display "$DISPLAY" >/dev/null 2>&1 && break
    sleep 1
done
if ! xdpyinfo -display "$DISPLAY" >/dev/null 2>&1; then
    echo "[install] ERROR: display $DISPLAY never became ready" >&2
    exit 1
fi
...
# -- 3. Install Windows Python (embeddable zip) ------------------------------
...
python3 -m pip download --quiet --only-binary=:all: --platform=win_amd64 --python-version=311 --implementation=cp -d /tmp/wheels MetaTrader5 rpyc numpy
